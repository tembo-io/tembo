...
use dataplane_webserver::routes::secrets::{get_secret_v1, update_postgres_password, get_secret_names_v1};
use serde_json::json;
...
async fn test_get_secret_names_v1() {
    ...
    assert!(!body.as_array().unwrap().is_empty(), "Response body is empty");
    ...
    assert!(secret_names.contains(&"app-role".to_string()), "app-role not found in response");
    assert!(secret_names.contains(&"readonly-role".to_string()), "readonly-role not found in response");
    assert!(secret_names.contains(&"superuser-role".to_string()), "superuser-role not found in response");
    assert!(secret_names.contains(&"certificate".to_string()), "certificate not found in response");
}
...
async fn test_get_secret_v1() {
    ...
    assert!(body.get("username").is_some(), "username not found in response");
    assert!(body.get("password").is_some(), "password not found in response");
}