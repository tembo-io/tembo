...
async fn fetch_metrics_loop(config: &Configuration, env: Environment, instance_settings: HashMap<String, InstanceSettings>) -> Result<()> {
    let client = reqwest::Client::new();

    let mut headers = HeaderMap::new();
    headers.insert("Accept", "application/json".parse()?);
    let jwt_token = config.bearer_access_token.as_ref().expect("JWT Token is not configured");
...
async fn fetch_metric(metric_query: &str, namespace_encoded: &str, client: &reqwest::Client, headers: &HeaderMap) -> Result<MetricsResponse> {
    const BASE_URL: &str = "https://api.data-1.use1.tembo.io";
    let query_encoded = urlencoding::encode(metric_query);
    let url = format!("{}/{}/metrics/query?&query={}", BASE_URL, namespace_encoded, query_encoded);
...
async fn get_instance_org_name(config: &Configuration, env: &Environment, instance_name : &String,) -> String {
    let instance_id = get_instance_id(instance_name, &config, &env).await.unwrap().unwrap();
    let instance = get_instance(&config, &env.org_id.clone().unwrap(), &instance_id).await.unwrap();
